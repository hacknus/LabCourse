import numpy as np
from constants import u0,std,a


def analytical_solution_dirichlet(t,x):
    return u0*std/np.sqrt(std**2+4*a*t)*np.e**((x**2/(4*a*t))*(std**2/(std**2+4*a*t)-1))

def analytical_dirichlet(delta_z,delta_t,number_z,number_t):
    ...
    for i in np.arange(number_t):
        if i*delta_t < 0.01:
            ...
            grid[i]=analytical_solution_dirichlet(grid_t[i],grid_z-0.5*(number_z-1)*delta_z)
